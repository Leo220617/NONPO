...
function Recuperar() {
    try {
        ...
        for (var i = 0; i < det.length; i++) {
            ...
            var totalT = 0;
            var oCargosT = 0;
            if (Pais == "P" || Pais == "N" || Pais == "D" || Pais == "H") {
    ...
    try {
        ...
        if ($("#CodProveedorH").val() == "" && ($("#DVD").val() == "" || $("#DVD").val() == undefined) && $("#NomProveedorH").val() == "") {
            ...
            $.ajax({
                ...
                data: { ids: $("#CodProveedorH").val(), dv: dv, nombre: $("#NomProveedorH").val() },
                        ...
                        success: function (json) {
                            ...
                            if (json.success == true) {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        window.location.href = window.location.href.split("/EditarH")[0];
                                    }
                                })
            ...
            case "0":
                {
                    if (document.getElementById("md_checkbox_11").checked) {
                        ...
                        var Precio0 = parseFloat(detalle.PrecioUnitario);
                        var Imp10 = Precio0 * 0.04;
                        detalle.ImpuestoMonto += Precio0 * 0.04;
                        detalle.Impuesto8 += Imp10;
                    }
                    else {
                        detalle.ImpuestoMonto += parseFloat(detalle.PrecioUnitario) * 0.00;
                    }
...
function onChangeTipoGastoE() {
    try {
        ...
        if (selected.toUpperCase().includes("Comb".toUpperCase()) && (Pais == "P" || Pais == "N" || Pais == "D" || Pais == "H")) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result != null) {
                    ...
                    var bandera14por = result.detCompras[0].subTotal * (result.detCompras[0].impuestoTarifa / 100);
                    ...
                    if (bandera14por < result.detCompras[0].impuestoMonto) {
    ...
    try {
        if (Pais == "P") {
            ...
            switch ($("#idClienteE").val()) {
                case "01":
                    {
                        $("#CodClienteE").inputmask({ "mask": "9999-9999-999999" });
                        break;
                    }
                case "02":
                    {
...
function maskE2() {
    try {
        switch ($("#selectTPE").val()) {
            case "01":
                {
                    $("#CodProveedorE").inputmask({ "mask": "9999-9999-999999" });
                    break;
                }
            case "02":
                {
...
function GenerarE() {
    try {
        ...
        var NumFacturaEH = $("#NumFacturaE").val();
        NumFacturaEH = NumFacturaEH.substring(NumFacturaEH.length - 4, NumFacturaEH.length);
        var EncCompras =
        {
            id: $("#idFacE").val(),
            ...
            TipoIdentificacionCliente: "0",
            NumFactura: NumFacturaEH,
            CodProveedor: (Pais == "P" || Pais == "H" || Pais == "D" ? $("#CodProveedorE").val() + "[" + $("#DVE").val() : $("#CodProveedorE").val()),
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    if (json.success == true) {
                        ...
                        objetoNuevo.Impuesto8 = impuesto8T;
...
function ValidarFacturaE() {
    try {
        ...
        //else if (document.getElementById("md_checkbox_15").checked && document.getElementById("md_checkbox_11").checked) {
